...
convert2array <- function(input,SamplesTotal) {
  vector <- unlist(input)
  allnames <- names(vector)
  parsed_vector <- strsplit(allnames,"\\.")
  
  transitions.unique <- unique(unlist(lapply(input, function(sample) {
    unlist(lapply(sample, function(molecule) {
      names(molecule[[1]])
    }))
  }))) #transition order
  
  reordered_indices <- order(match(sapply(parsed_vector, `[`, 3), names(input[[1]][[1]])),# corrMat
                             match(sapply(parsed_vector, `[`, 1), names(input)),  # sample
                             match(sapply(parsed_vector, `[`, 4), transitions.unique), #transition
                             sapply(parsed_vector, `[`, 2))  # molecule
  
  vector <- vector[reordered_indices]
  
  finalarray <- array(vector,dim=c(length(transitions.unique),SamplesTotal,6),dimnames = list(transitions.unique,names(input),names(input[[1]][[1]])))
...
#TRANSOPOSE BACK AND ADD OBSERVATION INFORMATION

TransposeAddObservationInfo <- function(CorrectedDataOutput, NamesCorrectedOutput, ObservationInfo){
  for (name in NamesCorrectedOutput$CorrectedDataNames) {
    data = CorrectedDataOutput[[name]]
    tdata = as.data.frame(t(data))
    tdata$spotId = as.numeric(gsub("X", "", rownames(tdata)))
    merge_data <- merge(ObservationInfo, tdata, by.x='spotId', by.y='spotId')
    CorrectedDataOutput[[name]] <- merge_data
  }
  return(CorrectedDataOutput)
}
