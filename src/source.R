...
NIAStripping <-
  function(MeasurementFile = NA,
    ...
    tryCatch({
      ...
      CorrectedDataOutputTrans <- TransposeAddObservationInfo(CorrectedDataOutput, NamesCorrectedOutput, ObservationInfo)
      
      # WRITE CORRECTED DATA TO FILE
      
      writeCorrectionResultsToFile(
        CorrectedDataOutput = CorrectedDataOutputTrans,
        NamesCorrectedOutput = NamesCorrectedOutput,
        logEnvironment = log.env,
        verbose = verbose
      )
