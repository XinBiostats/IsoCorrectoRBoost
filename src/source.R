...
RawDataCorrectionOnCompleteDataset <- function(MeasurementInfo, MoleculeInfo, CombinedProbList, MoleculesTotal, SamplesTotal, NamesCorrectedOutput, roundDigits, logEnvironment, parallel, verbose) {
  ...
  if (parallel){
    ...
    # Use mclapply to process samples in parallel on Mac
    # Use parlapply to process samples in parallel on Windows
    if (Sys.info()["sysname"] == "Windows") {
      cl <- parallel::makeCluster(parallel::detectCores() - 1)
      parallel::clusterExport(cl, c("MeasurementInfo",
                                    "MoleculeInfo",
                                    "CombinedProbList",
                                    "MoleculesTotal",
                                    "NamesCorrectedOutput",
                                    "roundDigits",
                                    "logEnvironment",
                                    "verbose",
                                    "RawDataCorrection",
                                    "errorHandler"),
                              envir = environment())
      CorrectionResultList <- parallel::parLapply(cl, seq_len(SamplesTotal), processSample)
